...
getwd()
temp = read.csv("port2_cv.csv",header=FALSE)
Dmat <- as.matrix(temp,rownames.force = NA)

p1 <- read.csv("~/Desktop/InterIIT Tech/code_csv/p2.csv",header = TRUE)
dvec <- c(p1$return)
dvecMatrix <- matrix(dvec)
dvecMatrixTranspose <- t(dvecMatrix)
originalReturnRisk <- read.csv("~/Desktop/InterIIT Tech/code_csv/portef2.csv",header = TRUE)



A <- matrix(c(rep(1,85)), ncol=1)
Amat <- cbind(A, diag(85), -diag(85))
class(Amat)
bvec <- c(1,rep(0,85),rep(-1,85))
...
table <- data.frame(matrix(NA_real_, nrow = 88, ncol = 2000))
count <- 1
while(count <=2000){
  lambda <- count/2000
...
write.table(table, file ="weightsReturnRiskRatio2.csv",sep=",",row.names=FALSE,col.names=FALSE)
