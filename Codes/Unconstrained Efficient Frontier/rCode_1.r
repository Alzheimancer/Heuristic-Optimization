...
getwd()
temp = read.csv("port1_cv.csv",header=FALSE)
Dmat <- as.matrix(temp,rownames.force = NA)

p1 <- read.csv("p1.csv",header = TRUE)
dvec <- c(p1$return)
dvecMatrix <- matrix(dvec)
dvecMatrixTranspose <- t(dvecMatrix)
originalReturnRisk <- read.csv("portef1.csv",header = TRUE)



A <- matrix(c(rep(1,31)), ncol=1)
Amat <- cbind(A, diag(31), -diag(31))
class(Amat)
bvec <- c(1,rep(0,31),rep(-1,31))
...
table <- data.frame(matrix(NA_real_, nrow = 34, ncol = 2000))
count <- 1
while(count <=2000){
  lambda <- count/2000
...
write.table(table, file ="weightsReturnRiskRatio1.csv",sep=",",row.names=FALSE,col.names=FALSE)
