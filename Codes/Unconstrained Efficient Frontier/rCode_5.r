...
getwd()
temp = read.csv("port5_cv.csv",header=FALSE)
Dmat <- as.matrix(temp,rownames.force = NA)

p1 <- read.csv("~/Desktop/InterIIT Tech/code_csv/p5.csv",header = TRUE)
dvec <- c(p1$return)
dvecMatrix <- matrix(dvec)
dvecMatrixTranspose <- t(dvecMatrix)
originalReturnRisk <- read.csv("~/Desktop/InterIIT Tech/code_csv/portef5.csv",sep=" ",header = TRUE)



A <- matrix(c(rep(1,225)), ncol=1)
Amat <- cbind(A, diag(225), -diag(225))
class(Amat)
bvec <- c(1,rep(0,225),rep(-1,225))
...
table <- data.frame(matrix(NA_real_, nrow = 228, ncol = 2000))
count <- 1
while(count <=2000){
  lambda <- count/2000
...
write.table(table, file ="weightsReturnRiskRatio5.csv",sep=",",row.names=FALSE,col.names=FALSE)
